...
public class LanguageManager {
    ...
    public void reloadLanguages() {
        ...
        boolean localeChanged = !previousDefaultLocale.equals(this.defaultLocale);

        if (localeChanged) {
            plugin.getLogger().info("Language changed from " + previousDefaultLocale + " to " + this.defaultLocale);

            // Remove the previous default locale data if it's not being used anymore
            if (!activeLocales.contains(previousDefaultLocale)) {
                localeMap.remove(previousDefaultLocale);
            }

            // Load the new default locale if it's not already loaded
            if (!localeMap.containsKey(this.defaultLocale)) {
                loadLocale(this.defaultLocale, activeFileTypes.toArray(new LanguageFileType[0]));
                activeLocales.add(this.defaultLocale);
            }
        } else {
            // Just reload the current default locale
            loadLocale(this.defaultLocale, activeFileTypes.toArray(new LanguageFileType[0]));
        }
